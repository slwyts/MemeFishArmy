...
<template>
  <div class="space-y-36">
    ...
    <div class="relative mt-8 max-w-sm mx-auto group">
    <div class="absolute -inset-0.5 bg-gradient-to-r from-amber-500 to-yellow-600 rounded-xl blur-lg opacity-60 group-hover:opacity-80 transition duration-500 animate-tilt"></div>
    <div class="relative bg-slate-900/80 backdrop-blur-sm p-4 rounded-xl border border-slate-700 shadow-xl">
      <div class="flex items-center justify-center space-x-2 mb-4">
        <ShieldCheckIcon class="h-6 w-6 text-amber-400" />
        <h2 class="text-xl font-bold text-white">{{ t('home.mint_card.title') }}</h2>
      </div>

      <div v-if="walletStore.isConnected" class="space-y-2 mb-4 text-left">
        <div class="flex items-center justify-between bg-slate-800/50 p-2 rounded-md border border-slate-700">
          <span class="font-semibold text-gray-300 text-sm">{{ t('home.mint_card.status_whitelist') }}</span>
          <div class="flex items-center space-x-1">
            <span :class="contractStore.isWhitelisted ? 'text-green-400' : 'text-red-400'" class="font-bold text-xs">{{ contractStore.isWhitelisted ? t('home.mint_card.status_whitelisted') : t('home.mint_card.status_not_whitelisted') }}</span>
            <CheckCircleIcon v-if="contractStore.isWhitelisted" class="h-4 w-4 text-green-400" />
            <XCircleIcon v-else class="h-4 w-4 text-red-400" />
          </div>
        </div>
        <div class="flex items-center justify-between bg-slate-800/50 p-2 rounded-md border border-slate-700">
          <span class="font-semibold text-gray-300 text-sm">{{ t('home.mint_card.status_minted') }}</span>
          <span class="font-mono text-base font-bold text-white">{{ contractStore.userMintCount }} / {{ contractStore.maxMintsPerUser }}</span>
        </div>
      </div>
            
      <p v-else class="text-gray-400 mb-4 h-[60px] flex items-center justify-center text-sm">{{ t('home.mint_card.status_connect_prompt') }}</p>

      <button @click="handleMintAction" :disabled="isMintDisabled" class="w-full py-2 font-bold text-base text-slate-900 rounded-md transition duration-300 ease-in-out transform hover:scale-105 bg-gradient-to-r from-amber-400 to-yellow-500 hover:from-amber-300 hover:to-yellow-400 focus:outline-none focus:ring-2 focus:ring-amber-500/50 disabled:from-gray-600 disabled:to-gray-700 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:text-gray-400">
        {{ mintButtonText }}
      </button>
            
      <div class="mt-2 text-xs min-h-[24px]">
